...
- raw.financials.*의 값은 **백만원 단위**. 직접 변환하지 말고 반드시
  formatted_financials의 문자열을 그대로 인용한다 (예: "258.94조", "65,669.76억").
  raw 숫자를 직접 변환하면 자릿수 오류가 잦으니 절대 금지.
- 증감률/비율(%)이나 ratio 계산은 raw 숫자나 computed.ratios 사용 OK
  (이건 단위 변환 아님).
...
export function buildDataContext(
  ...
  const formattedFinancials = {
    income_statement: formatFinancialBlock(
      raw.financials.income_statement as unknown as Record<string, unknown>
    ),
    balance_sheet: formatFinancialBlock(
      raw.financials.balance_sheet as unknown as Record<string, unknown>
    ),
    ...(raw.financials.cash_flow_raw
      ? {
          cash_flow_raw: formatFinancialBlock(
            raw.financials.cash_flow_raw as unknown as Record<string, unknown>
          ),
        }
      : {}),
  };
...
※ raw_financials의 각 line item을 한국식 단위로 미리 변환해둔 lookup.
※ narrative에 금액 인용할 때는 raw 숫자를 다시 변환하지 말고 이 문자열을 그대로 사용.
※ 연도 인덱스는 meta.fiscal_years와 동일 (0=첫 해, 마지막=최신).
${JSON.stringify(formattedFinancials, null, 2)}
</formatted_financials>
...
const PAGE_NARRATIVE_SCHEMA = `{
  ...
  "insight": {
    "conclusion": "<1~2문장 페이지 종합 결론>",
    "evidence": ["<5~10개 항목, 각 항목 한 줄. 데이터 인용 + 짧은 의미>"],
    "reasoning": "<3~5문장 narrative>",
    "accounting": ["<3~7개 K-IFRS 회계 watchpoint>"],
    "mna": ["<3~7개 M&A 관점>"],
    "monitoring": ["<3~7개 추적 지표>"]
  }
}`;
