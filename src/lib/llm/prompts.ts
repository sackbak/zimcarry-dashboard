...
3. 마크업 — 부정/긍정 신호 강조용 마커 2종:
   부정·위험 강조: ==텍스트== (양쪽 ==로 감싼다, 빨강 표시됨)
     예: "매출이 ==감소세 진입==하여 ==자본잠식 위험==이 커짐"
   긍정·기회 강조: ++텍스트++ (양쪽 ++로 감싼다, 파랑 표시됨)
     예: "FCF가 ++첫 흑자 전환++하며 ++운전자본 회수 가속==됨" (각 마커는 본인 짝과 매칭)
     예: "매출 ++46.8% YoY++로 ++외형 성장 회복++"
   잘못된 예 (절대 금지):
     - "==red==매출 감소==red==" (HTML 흉내 태그 X)
     - "==text===red==" (중첩 X)
     - 긍정 내용에 ==쓰기 X (==는 부정 전용)
     - 부정 내용에 ++쓰기 X (++는 긍정 전용)
   그 외 마크다운(**bold**, *italic*, # heading, - list) 절대 금지. 평문 + 위 마커만.
   부정·긍정이 모호한 일반 사실은 마커 없이 평문으로.
...
item_notes 대상 키 (formatted_financials.balance_sheet에 있는 것만):
...
※ 데이터에 실제로 있는 키에 대해서만 생성. 없는 키는 생략.`;
...
item_notes 대상 키 (formatted_financials.income_statement에 있는 것만):
