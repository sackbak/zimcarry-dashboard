...
- evidence는 5~7개, 각 항목은 한 줄 안에 구체적 숫자 + 의미.
- reasoning은 3~5문장 narrative — 단순 나열이 아닌 인과관계 서술.
- accounting/mna/monitoring 각 3~6개.`;
...

{
  ...
  "message": "<2~3문장 보조 설명. headline이 말한 테마의 배경과 규모 구체화>",
  "insight": {
    "conclusion": "<2~3문장 결론. 이 회사의 핵심 투자 테제 또는 리스크를 한 단락으로. 수치 포함 필수.>",
    "evidence": ["<5~7개 항목. 각 항목 한 줄: 구체적 숫자 + 그게 왜 중요한지. 단순 수치 나열 금지>"],
    "reasoning": "<3~5문장 인과관계 서술 — 숫자들이 어떤 구조적 문제/기회를 가리키는지, 이 패턴이 왜 이례적인지>",
    "accounting": ["<3~6개 K-IFRS 회계 watchpoint — 실제로 사업보고서에서 확인해야 할 주석 포인트>"],
    "mna": ["<3~6개 M&A 관점 — EV/Sales, EV/EBITDA 적용 시 밸류 범위, 청산가치 vs 장부가, 우선주 처리, 협상 레버리지 등>"],
    "monitoring": ["<3~6개 분기별 추적 지표 — 어떤 숫자가 어느 방향으로 움직이면 thesis가 바뀌는지>"]
  }
}
...
const PAGE_NARRATIVE_SCHEMA = `{
  "headline": "<1줄, 가장 두드러진 테마. 위험 부분만 ==강조text== 식으로 감싸기. ==red== 같은 태그 형식 금지. bold/italic/heading 등 일체 금지. 80자 이내>",
  "message": "<2~3문장 보조 설명. 구체적 수치 포함>",
  "insight": {
    "conclusion": "<2~3문장 결론. 수치 포함 필수>",
    "evidence": ["<5~7개 항목, 각 항목 한 줄. 구체적 숫자 + 의미. 단순 수치 재확인 금지>"],
    "reasoning": "<3~5문장 인과관계 서술>",
    "accounting": ["<3~6개 K-IFRS 회계 watchpoint>"],
    "mna": ["<3~6개 M&A 관점>"],
    "monitoring": ["<3~6개 추적 지표>"]
  }
}`;
